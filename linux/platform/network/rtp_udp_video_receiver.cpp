...
RtpUdpVideoReceiver::~RtpUdpVideoReceiver() {
    close();
}

Result<void> RtpUdpVideoReceiver::open(const RtpUdpVideoReceiverConfig& config) {
    if (config.max_datagram_size < kRtpFixedHeaderSize + 1) {
        return Result<void>::err(
            ErrorDomain::Config,
            ErrorCode::InvalidConfig,
            "RTP/UDP receiver max datagram size is too small");
    }
    config_ = config;
...
Result<MediaPacket> RtpUdpVideoReceiver::read_frame() {
    if (!is_open()) {
        return Result<MediaPacket>::err(
            ErrorDomain::Internal, ErrorCode::InvalidState, "RTP/UDP receiver is not open");
    }
    ...
    return Result<MediaPacket>::err(
        ErrorDomain::Network, ErrorCode::NetworkDisconnected, "RTP/UDP receiver closed");
}

void RtpUdpVideoReceiver::close() {
    socket_.close();
}

void RtpUdpVideoReceiver::interrupt() {
    close();
}

RtpUdpVideoReceiverStats RtpUdpVideoReceiver::stats() const {
    RtpUdpVideoReceiverStats s;
