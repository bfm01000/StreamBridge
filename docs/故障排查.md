...
- 自动化：`ctest -R av_sync` —— `av_sync_capture_test` 走生产链路录制 10s 到 `source/av_sync_test.flv` 并离线分析（判定阈值见 `docs/测试指南.md`）。修复后实测：start skew +20ms、drift +9ms、v/a 最大间隔 45/78ms、PASS。
