...
> 配套文档：
> - `docs/公共接口.md` — 公共层完整接口体系（类型、采集、编解码、传输、输出、时钟、同步、队列、会话、工厂、日志）
> - `docs/linux-总体架构.md` — Linux 端详细架构（推流实现、播放端设计、线程模型、错误处理、构建、测试）
> - `docs/时间戳与音视频同步.md` — 时间戳与音视频同步
> - `docs/里程碑.md` — 里程碑拆分
> - `docs/项目协作与决策日志.md` — 双 AI 协作
...
- FFmpeg 建议以预编译 Android ABI 包接入，包来源、版本、configure 参数写入 `docs/构建与运行.md`。
