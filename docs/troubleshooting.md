...
## 问题 12：Linux 播放端黑屏 + 渲染仅 2fps（video-only 流）

**现象**：

- ffplay 播放同一 RTMP 流画面正常；
- `streambridge_player` 窗口黑屏，`rendered` 计数极慢（10s 仅 24 帧），
  `av_diff_us` 持续增大（+347ms → +818ms），压缩包队列持续满载。

**根因**：

`MediaClock::start()` 只被音频线程调用。对 **video-only 流**（如 Android 推的
摄像头流，暂无音频轨），主时钟从未启动，`now()` 恒返回 0。每一帧的
`av_diff = pts - 0` 都为正且越来越大 → 同步决策全是 `Wait`，等待时长随
pts 增长（数百 ms），渲染几乎停滞；黑屏是因为播放器永远停留在流的
**前几帧**（摄像头启动期的暗帧），而 ffplay 不做同步等待、很快播放到正常画面。

**修复**：

视频线程首帧时若时钟未启动，则以该帧 PTS 启动单调主时钟：

```cpp
if (!clock_started) {
    p.clock.start(pts);   // 无音频流：以首视频帧启动时钟
    clock_started = true;
}
```

有音频的流不受影响（音频线程会随后接管时钟并切换到音频主时钟）。

**验证**：

修复后 `rendered` 增速 ≈ 30fps（源帧率 29.75），`av_diff_us` 稳定 +55~61ms，
`dropped=0`。排查手段：将播放端解码的首帧 RGBA 与 `ffmpeg -pix_fmt rgba`
解码同一流的首帧逐像素对比，确认画面内容一致、排除解码/渲染环节。

**经验**：

- 无音频流时主时钟必须有一个显式的启动路径，任何「主时钟未启动」状态
  都必须有可定义的行为，不能让同步决策在 0 时钟上运行。

---

## 问题 13：Linux 播放端窗口黑屏——SDL software renderer 的 RGBA8888 纹理字节序缺陷

**现象**：

- 播放器指标一切正常（rendered 30fps、av_diff 稳定、音频在播）；
- 但 SDL 窗口始终黑屏（实际是颜色严重错位、蓝通道丢失后接近黑色）；
- ffplay 播放同一流画面正常。

**排查过程**（地面真值法）：

1. 逐像素对比播放器解码首帧与 `ffmpeg -pix_fmt rgba` 解码同一流 → 数据一致，
   排除解码链路；
2. 给播放器加 `--test-pattern`（纯红/绿/蓝/白四色帧）并回读窗口 surface：
   红→黑、绿→红、蓝→绿、白→黄；
3. 结论：SDL software renderer 创建 **RGBA8888** 纹理时不做字节序转换，
   把上传数据直接按 **ARGB** 解释（`[A][R][G][B]` 位序），导致蓝通道丢失、
   通道整体错位。

**修复**：

- 纹理格式改用 `SDL_PIXELFORMAT_ARGB8888`（software renderer 官方支持列表内）；
- 渲染缓冲按 ARGB 内存序打包：`0xAARRGGBB`（LE 字节 `[B][G][R][A]`）；
- 解码器输出的 RGBA 帧（LE 字节 `[R][G][B][A]`）交换 R/B 并强制 A=255 后上传。

**验证**：`--test-pattern` 四色全部正确；本地文件窗口像素 (88,86,82) 与
ffmpeg 解码地面真值完全一致。

**经验**：

- SDL software renderer 对纹理格式的支持并不完整，用未列在支持清单里的格式
